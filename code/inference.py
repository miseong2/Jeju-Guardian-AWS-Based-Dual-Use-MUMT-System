...
def model_fn(model_dir):
    ...
    # U-Net 로드
    sys.path.append(model_dir) # unet.py import를 위해 경로 추가
...
def predict_fn(job_id, models):
    ...
    # 3. 시각화용 캔버스(RGB 이미지) 생성
    logger.info("시각화용 RGB 캔버스 생성 중...")
    canvas_img = create_rgb_canvas(stacked_tif_path)
    
    all_detections = []
    with rasterio.open(stacked_tif_path) as src:
        # 4. 추론 실행 (캔버스에 그림 그리기 포함)
        # U-Net
        unet_dets = run_unet_inference(src, models['unet'], canvas_img)
        all_detections.extend(unet_dets)
        
        # YOLO (Ship)
        ship_dets = run_yolo_inference(src, models['yolo_ship'], 'ship', 0.3, canvas_img, (0, 0, 255)) # 빨강(BGR)
        all_detections.extend(ship_dets)

        # YOLO (Debris)
        debris_dets = run_yolo_inference(src, models['yolo_debris'], 'debris', 0.1, canvas_img, (0, 255, 255)) # 노랑(BGR)
        all_detections.extend(debris_dets)

    # 5. 결과 이미지 S3 업로드 (processed 폴더)
    result_img_name = f"{job_id}_result.jpg"
    result_img_path = os.path.join(WORK_DIR, result_img_name)
    
    # BGR -> RGB 변환 없이 저장 (OpenCV는 BGR 저장)
    cv2.imwrite(result_img_path, canvas_img)
    
    s3_key_out = f"processed/{result_img_name}"
    s3_client.upload_file(result_img_path, BUCKET, s3_key_out)
    logger.info(f"시각화 결과 업로드 완료: s3://{BUCKET}/{s3_key_out}")
...
def create_rgb_canvas(tif_path):
    """분석용 TIF 파일에서 RGB(4,3,2) 채널을 뽑아 시각화 이미지를 만듭니다."""
    with rasterio.open(tif_path) as src:
        # Band 4(R), 3(G), 2(B) 읽기
        # 메모리 효율을 위해 전체를 한 번에 읽지 않고 다운샘플링 할 수도 있으나,
        # 정밀도를 위해 원본 해상도 유지 (인스턴스 메모리가 충분해야 함)
        r = src.read(4)
        g = src.read(3)
        b = src.read(2)
        
        rgb = np.dstack((b, g, r)) # OpenCV는 BGR 순서 사용
        
        # 밝기 보정 (Sentinel-2 3000값 기준)
        rgb_norm = np.clip((rgb.astype(np.float32) / 3000.0) * 255.0, 0, 255).astype(np.uint8)
        
        # 메모리 정리를 위해 변수 삭제
        del r, g, b
        return rgb_norm.copy()
...
def stack_bands(input_folder, output_filename, target_order):
    ...
    if 'B8A' in file_map and 'B08' in file_map and file_map['B08'] == file_map['B8A']:
         for f in all_files:
            if 'B08' in f and 'B8A' not in f:
                file_map['B08'] = os.path.join(input_folder, f)
                break
...
def run_unet_inference(src, model, canvas):
    logger.info("U-Net 추론 중...")
    detections = []
    ...
    for y in range(0, h, win):
        for x in range(0, w, win):
            ...
            targets = [(4, "ship", (0, 0, 255)), (0, "debris", (0, 255, 255))] 
            for cid, label, color in targets:
                ...
                for c in cnts:
                    ...
                    # 1. 캔버스에 그리기 (Global 좌표)
                    # 현재 윈도우의 위치(x,y)를 더해줘야 전체 이미지상에서 올바른 위치에 그려짐
                    c_global = c.copy()
                    c_global[:, :, 0] += x
                    c_global[:, :, 1] += y
                    
                    bx, by, bw, bh = cv2.boundingRect(c_global)
                    cv2.rectangle(canvas, (bx, by), (bx+bw, by+bh), color, 2)
                    cv2.putText(canvas, label, (bx, by-5), font, 0.5, color, 1)
                    ...
                    detections.append({"class": f"unet_{label}", "lat": lat, "lon": lon, "conf": float(probs[cid, cy, cx])})
    return detections

def run_yolo_inference(src, model, object_type, conf_thresh, canvas, color):
    logger.info(f"YOLO ({object_type}) 추론 중...")
    detections = []
    h, w = src.height, src.width
    win = 640
    font = cv2.FONT_HERSHEY_SIMPLEX
        ...
        for x_off in range(0, w, win):
            ...
            if results[0].boxes is not None:
                for box in results[0].boxes.data.cpu().numpy():
                    ...
                    # 1. 캔버스에 그리기
                    # 로컬 윈도우 좌표(x1, y1)를 글로벌 좌표로 변환
                    gx1, gy1 = int(x1) + x_off, int(y1) + y_off
                    gx2, gy2 = int(x2) + x_off, int(y2) + y_off
                    
                    cv2.rectangle(canvas, (gx1, gy1), (gx2, gy2), color, 2)
                    cv2.putText(canvas, f"{object_type}", (gx1, gy1-5), font, 0.5, color, 1)
                    
                    # 2. 좌표 계산 및 결과 저장
                    cx, cy = (x1 + x2) / 2, (y1 + y2) / 2
                    lat, lon = get_lat_lon(x_off + cx, y_off + cy, src)
                    detections.append({"class": f"yolo_{object_type}", "lat": lat, "lon": lon, "conf": float(conf)})
    return detections
